...
默认从 GitHub 项目 Solana-MEV-Bot-Blackbook 获取（社区维护的疑似机器人列表）。

支持用户通过本地文件 address_pool.txt 提供自定义地址（每行一个，支持 # 注释）。

链上数据：

通过 Helius RPC 获取地址的签名历史及交易详情。需自行申请 API Key。
...
MODE=feb   # 可选 feb 或 default（见下文）
（可选）自定义地址池：

复制 address_pool.txt.example 为 address_pool.txt，并按格式填入您感兴趣的地址（每行一个，支持 # 注释）。
